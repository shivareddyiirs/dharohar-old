	...
	, m_cloud(cloud)
{
    ...
    connect(generateRasterPushButton,	SIGNAL(clicked()),					this,	SLOT(saveToDB()));
...
void ccRasterizeTool::resampleOptionToggled(bool )
{
...
void ccRasterizeTool::sfProjectionTypeChanged(int )
{
	gridIsUpToDate(false);
}

void ccRasterizeTool::projectionDirChanged(int )
{
	updateGridInfo();
	gridIsUpToDate(false);
}
...
void ccRasterizeTool::activeLayerChanged(int layerIndex, bool autoRedraw/*=true*/)
{
	if (layerIndex != 0) //0 is always the cell height
	{
        ...
        generateRasterPushButton->setEnabled(false);
	...
	else
	{
        ...
        generateRasterPushButton->setEnabled(true);
...
bool ccRasterizeTool::ExportGeoTiff(QString ,
                                    const ExportBands& ,
...
void ccRasterizeTool::generateImage() const
{
	...
	if (!bitmap8.isNull())
	{
		...
		//open file saving dialog
		{
            ...
            QString outputFilename = ImageFileFilter::GetSaveFilename(	"Export raster image to file",
																		"raster_image",
																		imageSavePath,
			...
			if (!outputFilename.isNull())
			{
                ...
                /*QStringList filename = outputFilename.split("/");
                if (bitmap8.save("temp" + filename[filename.size() - 1]))
                {
                    system(("gdal_translate -expand rgb \"temp" + filename[filename.size() - 1] + "\" \"" + outputFilename + "\" & pause").toStdString().c_str());
                    remove(("temp" + filename[filename.size() - 1]).toStdString().c_str());
...
void ccRasterizeTool::saveFailed(){
    ccLog::Print("Save operation successful.");
    this->generateRasterPushButton->setText("Save Project");
    this->generateRasterPushButton->setEnabled(true);
    return;
}

void ccRasterizeTool::saveDone(){
    QMessageBox msgBox;
    msgBox.setText("Save operation successful.");
...
void ccRasterizeTool::saveToDB()
{
    ...
    if (!bitmap8.isNull())
    {
        ...
        QString text;
        while(true){
            text = QInputDialog::getText(this, tr("Save Project"),
                                                 tr("Saving project into database.\nEnter a file name for raster:"), QLineEdit::Normal,
                                                 QDir::home().dirName(), &ok);
            if(ok && !text.isEmpty()){
                if(!(std::ifstream(text.toStdString() + ".tif")))
                    break;
                else{
                    QMessageBox msgBox;
                    msgBox.setText("Cannot use this file name. Try again.");
                    msgBox.setWindowTitle("Filename Error");
                ...
                QMessageBox msgBox;
                msgBox.setText("Filename cannot be blank. Try again.");
                msgBox.setWindowTitle("Filename Error");
                msgBox.setIcon(msgBox.Critical);
                msgBox.exec();
            }
                ...
                QMessageBox msgBox;
                msgBox.setText("Save operation cancelled by user.");
                msgBox.setWindowTitle("Operation Cancelled");
                msgBox.setIcon(msgBox.Information);
                msgBox.exec();
                return;
            }
        ...
        imagefilename = text + ".tif";

        QMessageBox msgBox;
        msgBox.setText("Save operation in progress in the background.\nPlease do not close this window, it will interrupt the operation.\nYou will be notified when its done.");
        msgBox.setWindowTitle("Saving is in Progress");
        msgBox.setIcon(msgBox.Information);
        msgBox.exec();
        ccLog::Print("Save operation started.");
        ccLog::Print("Please do not close the application. Saving is in progress.");

        this->generateRasterPushButton->setText("Saving...");
        this->generateRasterPushButton->setEnabled(false);
        ...
        QString command;
        command = "cmd /c \"gdal_translate\" -expand rgb temp\\temp" + imagefilename + " temp\\" + imagefilename;
        wchar_t warray[250];
        wcsncpy(warray, command.toStdWString().c_str(), command.toStdWString().size());
        warray[command.toStdWString().size()] = '\0';

        bitmap8.save("temp/temp" + imagefilename);

        //system(("gdal_translate -expand rgb temp/temp" + imagefilename + " temp/" + imagefilename).toStdString().c_str());
        ...
        remove(("temp/temp" + imagefilename).toStdString().c_str());

        QString password, user, db, sql, host, port, project;

        user = ((MainWindow*)this->parent())->txtUser->text();
        db = ((MainWindow*)this->parent())->txtDBName->text();
        password = ((MainWindow*)this->parent())->txtPassword->text();
        port = ((MainWindow*)this->parent())->txtPort->text();
        host = ((MainWindow*)this->parent())->txtLocation->text();
        sql = "cmd /c psql \"user=" + user + " password=" + password + " dbname=" + db + " host=" + host + " port=" + port + "\" < ";
        project = ((MainWindow*)this->parent())->txtProjectName->text();

        saveThread *newThread = new saveThread(m_cloud, sql, project, imagefilename, db);
        connect(newThread, SIGNAL(savingDone()), this, SLOT(saveDone()));
        connect(newThread, SIGNAL(savingFailed()), this, SLOT(saveFailed()));
        newThread->start();
