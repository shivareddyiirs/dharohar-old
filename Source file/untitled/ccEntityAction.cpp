...
namespace ccEntityAction
{
				...
				if (sf && sf->currentSize() >= pc->size())
				{
					...
					else
					{
						...
						for (unsigned i = 0; i < pc->size(); ++i)
						{
                            ...
                            signed colIndex = static_cast<unsigned>((val - minSF) / step);
    ...
    bool	processMeshSF(const ccHObject::Container &selectedEntities, ccMesh::MESH_SCALAR_FIELD_PROCESS process, QWidget *)
	{
		for (ccHObject* ent : selectedEntities)
		{
