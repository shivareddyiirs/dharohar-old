...
int main(int argc, char **argv)
{
	...
	**/
    ...
    QString lang = argv[7];
    QTranslator t;
    if(lang.toLower().compare("english") != 0)
    {
        t.load("language script/" + lang + "qt.qm");
    ...
    if (commandLine)
    {
        ...
        if (argc > 7 && QString(argv[lastArgumentIndex]).toUpper() == "-LANG")
        {
	...
	else
	{
        ...
        mainWindow->txtProjectName->setText(QString(argv[1]));
        mainWindow->txtUser->setText(QString(argv[2]));
        mainWindow->txtPassword->setText(QString(argv[3]));
        mainWindow->txtLocation->setText(QString(argv[4]));
        mainWindow->txtDBName->setText(QString(argv[5]));
        mainWindow->txtPort->setText(QString(argv[6]));
		mainWindow->show();
		QApplication::processEvents();
